const langKo = {
  topad: [
    {
      nm: "하림펫푸드 제품 첫 구매 시 간식 100원 이벤트",
      cls: "bgactiveColor text-center p-1",
      href: ""
    }
  ],
  gnb: [
    {
      nm: "이벤트",
      href: "#slide-banner",
      cls: "text-dark position-relative",
      target: "_blank"
    },
    {
      nm: "하림펫푸드",
      href: "#section-brand",
      cls: "text-dark position-relative",
      target: "_blank"
    },
    {
      nm: "제품",
      href: "#section-item",
      cls: "text-dark position-relative",
      target: "_blank"
    },
    {
      nm: "후기",
      href: "#section-instagram",
      cls: "text-dark position-relative",
      target: "_blank"
    },
    {
      nm: "체험",
      href: "#form",
      cls: "text-dark position-relative",
      target: "_blank"
    }
  ],
  slidebanner: [
    {
      h3: "반짝 배송 OPEN",
      p: "오늘 주문, 반짝하면 새벽 도착!",
      href: ""
    },
    {
      h3: "설레는 봄 | 할인해 봄",
      p: "봄맞이 최대 50% 할인 혜택 받으세요",
      href: ""
    },
    {
      h3: "신선함이 가득한<br />당일생산",
      p: "하림펫푸드는 사료가 아니다<br />음식이다",
      href: ""
    },
    {
      h3: "하림펫푸드는<br />사료가 아니다 음식이다",
      p: "#하림팻푸드 댕냥이 동안선발대회<br />반려동물 자랑도 하고 간식도 받고!",
      href: ""
    },
    {
      h3: "멍이냥이얌얌<br />리뷰 EVENT",
      p: "하림펫푸드 제품 리뷰 작성시<br />추첨을 통해 간식 샘플 증정",
      href: ""
    },
  ],
  linebanner: {
    h2: "GOLD MEMBER 골드멤버8가지혜택",
    i: [
      {
        cls: "bi-balloon-heart",
      },
      {
        cls: "bi-gift",
      },
      {
        cls: "bi-box2-heart",
      },
      {
        cls: "bi-ticket-perforated",
      },
      {
        cls: "bi-truck",
      },
      {
        cls: "bi-envelope-check",
      },
      {
        cls: "bi-cash-coin",
      },
      {
        cls: "bi-headset",
      },
    ]
  },
  sectionbrand: {
    h2: "우리의 약속 | OUR COMMITMENT",
    brandlist: [
      {
        src: "./img/dongmin/form1.gif",
        h3: "하림펫푸드의 신선도는 | 냄새로 확인할 수 있습니다.",
        p: "보통 사료는 나쁜 냄새가 많이 나지만 | 하림펫푸드는 생고기와 야채 등신선한 재료로만 만들어 | 재료 본연의 고소한 향을 느낄 수 있습니다."
      },
      {
        src: "./img/dongmin/form2.gif",
        h3: "하림펫푸드는 100% | 식품 등급으로 만듭니다.  ",
        p: "보통 사료는 사람이 먹지 못하는 원료(육분 등)를 넣지만, | 하림펫푸드는 사람이 먹을 수 있는 생고기를 넣습니다."
      },
      {
        src: "./img/dongmin/form3.gif",
        h3: "하림펫푸드는 | 합성 보존제를 첨가하지 않습니다. ",
        p: "뜨겁고 습한 적도를 건너오는 사료의 경우  | 산패를 막기 위해방부제를 첨가합니다. | 하지만, 하림펫푸드는 어떠한 합성 보존제도 첨가하지 않았습니다."
      },
      {
        src: "./img/dongmin/form4.gif",
        h3: "하림펫푸드는 | 식품 생산 수준의 공장에서 만듭니다.",
        p: "일반 가축용 사료를 만드는 사료공장과는 달리, | 우리는 첨단 설비와관리 시스템을 갖추고 | 식품 생산수준의 공장에서 직접 만듭니다."
      },
      {
        src: "./img/dongmin/form5.gif",
        h3: "신선함을 약속하는 시간, | 제조일자를 표기합니다. ",
        p: "수입한 이후 소분 날짜와유통기한만 표기하지만, | 하림펫푸드는 더 안전하고 믿을 수 있도록 표기합니다."
      },
    ]
  },
  sectionitem: {
    mainlist: [
      {
        cls: "tap active",
        nm: "#우리 아이들도 100세까지",
        href: ".no0"
      },
      {
        cls: "tap",
        nm: "#간식 / 영양제",
        href: ".no1"
      },
      {
        cls: "tap",
        nm: "#휴먼그레이드 PICK",
        href: ".no2"
      },
      {
        cls: "tap",
        nm: "#가장 맛있는 시간 30일",
        href: ".no3"
      },
      {
        cls: "tap",
        nm: "#하림펫푸드 PICK ITEM",
        href: ".no4"
      }
    ],
    listgroup: [
      {
        cls: "col-6 no0",
        href: "#none",
        src: "./img/gyuwan/product/li1/list1_1.jpg",
        alt: "list1_1"
      },
      {
        cls: "col-6 no0",
        href: "#none",
        src: "./img/gyuwan/product/li1/list1_2.jpg",
        alt: "list1_2"
      },
      {
        cls: "col-6 no0",
        href: "#none",
        src: "./img/gyuwan/product/li1/list1_3.jpg",
        alt: "list1_3"
      },
      {
        cls: "col-6 no0",
        href: "#none",
        src: "./img/gyuwan/product/li1/list1_4.jpg",
        alt: "list1_4"
      },
      {
        cls: "col-6 no0",
        href: "#none",
        src: "./img/gyuwan/product/li1/list1_5.jpg",
        alt: "list1_5"
      },
      {
        cls: "col-6 no0",
        href: "#none",
        src: "./img/gyuwan/product/li1/list1_6.jpg",
        alt: "list1_6"
      },
      {
        cls: "col-6 no1",
        href: "#none",
        src: "./img/gyuwan/product/li2/list2_1.jpg",
        alt: "list2_1"
      },
      {
        cls: "col-6 no1",
        href: "#none",
        src: "./img/gyuwan/product/li2/list2_2.jpg",
        alt: "list2_2"
      },
      {
        cls: "col-6 no1",
        href: "#none",
        src: "./img/gyuwan/product/li2/list2_3.jpg",
        alt: "list2_3"
      },
      {
        cls: "col-6 no1",
        href: "#none",
        src: "./img/gyuwan/product/li2/list2_4.jpg",
        alt: "list2_4"
      },
      {
        cls: "col-6 no1",
        href: "#none",
        src: "./img/gyuwan/product/li2/list2_5.jpg",
        alt: "list2_5"
      },
      {
        cls: "col-6 no1",
        href: "#none",
        src: "./img/gyuwan/product/li2/list2_6.jpg",
        alt: "list2_6"
      },
      {
        cls: "col-6 no2",
        href: "#none",
        src: "./img/gyuwan/product/li2/list3_1.jpg",
        alt: "list3_1"
      },
      {
        cls: "col-6 no2",
        href: "#none",
        src: "./img/gyuwan/product/li2/list3_2.jpg",
        alt: "list3_2"
      },
      {
        cls: "col-6 no2",
        href: "#none",
        src: "./img/gyuwan/product/li2/list3_3.jpg",
        alt: "list3_3"
      },
      {
        cls: "col-6 no2",
        href: "#none",
        src: "./img/gyuwan/product/li2/list3_4.jpg",
        alt: "list3_4"
      },
      {
        cls: "col-6 no2",
        href: "#none",
        src: "./img/gyuwan/product/li2/list3_5.jpg",
        alt: "list3_5"
      },
      {
        cls: "col-6 no2",
        href: "#none",
        src: "./img/gyuwan/product/li2/list3_6.jpg",
        alt: "list3_6"
      },
      {
        cls: "col-6 no3",
        href: "#none",
        src: "./img/gyuwan/product/li2/list4_1.jpg",
        alt: "list4_1"
      },
      {
        cls: "col-6 no3",
        href: "#none",
        src: "./img/gyuwan/product/li2/list4_2.jpg",
        alt: "list4_2"
      },
      {
        cls: "col-6 no3",
        href: "#none",
        src: "./img/gyuwan/product/li2/list4_3.jpg",
        alt: "list4_3"
      },
      {
        cls: "col-6 no3",
        href: "#none",
        src: "./img/gyuwan/product/li2/list4_4.jpg",
        alt: "list4_4"
      },
      {
        cls: "col-6 no3",
        href: "#none",
        src: "./img/gyuwan/product/li2/list4_5.jpg",
        alt: "list4_5"
      },
      {
        cls: "col-6 no3",
        href: "#none",
        src: "./img/gyuwan/product/li2/list4_6.jpg",
        alt: "list4_6"
      },
      {
        cls: "col-6 no4",
        href: "#none",
        src: "./img/gyuwan/product/li2/list5_1.jpg",
        alt: "list5_1"
      },
      {
        cls: "col-6 no4",
        href: "#none",
        src: "./img/gyuwan/product/li2/list5_2.jpg",
        alt: "list5_2"
      },
      {
        cls: "col-6 no4",
        href: "#none",
        src: "./img/gyuwan/product/li2/list5_3.jpg",
        alt: "list5_3"
      },
      {
        cls: "col-6 no4",
        href: "#none",
        src: "./img/gyuwan/product/li2/list5_4.jpg",
        alt: "list5_4"
      },
      {
        cls: "col-6 no4",
        href: "#none",
        src: "./img/gyuwan/product/li2/list5_5.jpg",
        alt: "list5_5"
      },
      {
        cls: "col-6 no4",
        href: "#none",
        src: "./img/gyuwan/product/li2/list5_6.jpg",
        alt: "list5_6"
      },
    ]
  },
  sectioninstagram: {
    instagroup: [
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram1.jpg",
        alt: "인스타후기사진1"
      },
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram2.jpg",
        alt: "인스타후기사진2"
      },
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram3.jpg",
        alt: "인스타후기사진3"
      },
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram4.jpg",
        alt: "인스타후기사진4"
      },
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram5.jpg",
        alt: "인스타후기사진5"
      },
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram6.jpg",
        alt: "인스타후기사진6"
      },
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram7.jpg",
        alt: "인스타후기사진7"
      },
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram8.jpg",
        alt: "인스타후기사진8"
      },
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram9.jpg",
        alt: "인스타후기사진9"
      },
      {
        href: "#none",
        src: "./img/gyuwan/instagram/instagram10.jpg",
        alt: "인스타후기사진10"
      },
    ]
  },
}